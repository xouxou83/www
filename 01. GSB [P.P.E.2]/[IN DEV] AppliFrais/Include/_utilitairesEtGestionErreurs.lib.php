...
/** 
 * Fournit le libelle en francais correspondant à un numero de mois.                     
 *
 * Fournit le libelle francais du mois de numero $unNoMois.
 * Retourne une chaine vide si le numero n'est pas compris dans l'intervalle [1,12].
...
/** 
 * Verifie si une chaine fournie est bien une date valide, au format JJ/MM/AAAA.                     
 * 
 * Retrourne true si la chaine $date est une date valide, au format JJ/MM/AAAA, FALSE sinon.
 * @param string date à verifier
...
/**
 * Transforme une date au format francais jj/mm/aaaa vers le format anglais aaaa-mm-jj
 ...
 * @return string la date au format format francais jj/mm/aaaa
*/
 ...
 * 
 * Retourne la valeur de la donnee portant le nom $nomDonnee recue dans l'url, 
 ...
 * 
 * Retourne la valeur de la donnee portant le nom $nomDonnee recue dans le corps de la requete http, 
 ...
 * recue dans l'url ou corps de requete, 
 * $valDefaut si aucune donnee de nom $nomDonnee ni dans l'url, ni dans corps.
 * Si le meme nom a ete transmis à la fois dans l'url et le corps de la requete,
...
/**
 * Fonction de chargement du sommaire
 * @param none
 * @return String chemin d'acces au sommaire
 */
function sommaireUser()	{
	$sommaireUser = "./Include/_sommaire.inc.php";
	return $sommaireUser;
}


/*
 * Fonction permettant a chaque page de mettre GET Visiteur ou Comptable
 *
 */
function identifierPostUser()	{
	if (lireDonneeUrl('User') == "Visiteur") {
		$postUser = "<input type=\"hidden\" name=\"User\" value=\"Visiteur\"/>";
		$post = "<form action=\"\" method=\"GET\">".$postUser."</form>";
		return $post;
	}
	elseif (lireDonneeUrl('User') == "Comptable") {
		$postUser = "<input type=\"hidden\" name=\"User\" value=\"Comptable\"/>";
		$post = "<form action=\"\" method=\"GET\">".$postUser."</form>";
		return $post;
	}
	else {
