 ...
 */
  ...
  // acquisition des donnees entrees, ici le numéro de mois et l'etape du traitement
  ...
  if ($etape == "validerConsult") { // l'utilisateur valide ses nouvelles donnees
      ...
      // verification de l'existence de la fiche de frais pour le mois demandé
      $existeFicheFrais = existeFicheFrais($idConnexion, $moisSaisi, obtenirIdUserConnecte());
      // si elle n'existe pas, on la crée avec les élets frais forfaitisees à 0
      if ( !$existeFicheFrais ) {
          ajouterErreur($tabErreurs, "Le mois demandé est invalide");
      }
      else {
          // récupération des donnees sur la fiche de frais demandée
...
// demande et affichage des differents elements (forfaitisees et non forfaitisees)
// de la fiche de frais demandée, uniquement si pas d'erreur détecté au controle
...
?>
    ...
    <div class="encadre">
    <p>Montant validé : <?php echo $tabFicheFrais["montantValide"];?></p><?php          
            // demande de la requête pour obtenir la liste des elements 
            // forfaitisees du visiteur connecté pour le mois demandé
  	...
  	<table class="listeLegere">
  	   <caption>quantites des elements forfaitisees</caption>
        <tr>
            <?php
            // premier parcours du tableau des frais forfaitisees du visiteur connecté
            // pour afficher la ligne des libellés des frais forfaitisees
        ...
        <tr>
            <?php
            // second parcours du tableau des frais forfaitisees du visiteur connecté
            // pour afficher la ligne des quantites des frais forfaitisees
            foreach ( $tabEltsFraisForfait as $unLibelle => $uneQuantite ) {
            ?>
...
<?php          
            // demande de la requête pour obtenir la liste des elements hors
