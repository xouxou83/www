...
/** 
 * Vérifie la validité des donnees d'une ligne de frais hors forfait.
 *  
...
function verifierLigneFraisHF($date, $libelle, $montant, &$tabErrs) {
    ...
    if ($libelle == "") {
		ajouterErreur($tabErrs, "Le libellé doit etre renseigné.");
	}
	// verification du montant
	if ($montant == "") {
		ajouterErreur($tabErrs, "Le montant doit etre renseigné.");
	}
	elseif ( !is_numeric($montant) || $montant < 0 ) {
        ajouterErreur($tabErrs, "Le montant doit etre numérique positif.");
    }
    // verification de la date d'engagement
	if ($date == "") {
		ajouterErreur($tabErrs, "La date d'engagement doit etre renseignée.");
	}
	elseif (!estDate($date)) {
